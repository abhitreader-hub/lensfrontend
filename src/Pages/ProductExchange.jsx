...
const PRODUCT_EXCHANGE_VOUCHERS = [
  {
    id: 1,
    billSeries: "Exchange",
    billNo: "2108",
    billDate: "14-08-2025",
    partyName: "The Only Eye Ware Store",
    type: "Lens",
    exchInQty: "2.00",
    exchInAmnt: "0.00",
    exchOutQty: "2.00",
  ...
  },
  {
    ...
    billNo: "2109",
    billDate: "15-08-2025",
    partyName: "VisionMax Optical Solutions",
    type: "Frame",
    exchInQty: "1.00",
    exchInAmnt: "500.00",
    exchOutQty: "1.00",
  ...
  },
  {
    ...
    billNo: "2110",
    billDate: "16-08-2025",
    partyName: "Crystal Clear Optics",
    type: "Contact Lens",
    exchInQty: "5.00",
    exchInAmnt: "1200.00",
    exchOutQty: "3.00",
...
function ProductExchange() {
  ...
  const filteredVouchers = useMemo(() => {
    ...
    return PRODUCT_EXCHANGE_VOUCHERS.filter((v) => {
      if (
        q &&
        !`${v.billNo} ${v.partyName} ${v.billSeries} ${v.type} ${v.status}`.toLowerCase().includes(q)
      )
        return false;
      if (
        filters.billSeries &&
  ...
  const totals = useMemo(() => {
    ...
    filteredVouchers.forEach((v) => {
      totalExchInQty += parseNumber(v.exchInQty);
      totalExchInAmnt += parseNumber(v.exchInAmnt);
      totalExchOutQty += parseNumber(v.exchOutQty);
      totalExchOutAmnt += parseNumber(v.exchOutAmnt);
    });
      ...
      <div className="max-w-[98vw] mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            Manage product exchanges and transactions
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md border border-slate-200 p-4 mb-4">
          ...
          <div className="flex flex-wrap justify-between gap-2">
            <div className="flex flex-wrap justify-start gap-2">
              ...
              <Link to='/add/addproductexchange' className="inline-flex items-center gap-1.5 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors duration-200">
            ...
            <table className="min-w-full table-fixed divide-y divide-slate-200">
              <thead className="bg-gradient-to-r from-blue-50 to-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="min-w-[220px] text-center py-4 px-3 text-slate-700 font-bold text-sm">
                    Party Name
                  </th>
                  <th className="min-w-[100px] text-center py-4 px-3 text-slate-700 font-bold text-sm">
                    Type
                ...
                {filteredVouchers.length === 0 ? (
                  <tr>
                    <td colSpan="12" className="p-10 text-center text-slate-500">
                      <p className="text-xl">No exchange records found</p>
                      <p className="text-md">
                  ...
                  filteredVouchers.map((v, i) => (
                    ...
                    >
                      ...
                      <td className="text-center text-slate-900 font-medium py-5 px-3 align-top">
                        {v.exchInAmnt !== "0.00" ? `₹${v.exchInAmnt}` : "-"}
                      </td>
                      <td className="text-center text-slate-900 font-medium py-5 px-3 align-top">
                        {v.exchOutQty}
                      </td>
                      <td className="text-center text-slate-900 font-medium py-5 px-3 align-top">
                        {v.exchOutAmnt !== "0.00" ? `₹${v.exchOutAmnt}` : "-"}
                      </td>
                      <td className="py-5 px-3 align-top text-center">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(v.status)}`}>
                          {v.status}
                        </span>
                      </td>
              ...
              <tfoot className="bg-slate-50">
                <tr>
                  ...
                  <td className="py-3 px-3 text-sm font-bold text-slate-800 text-center">
                    {totals.totalExchInQty.toFixed(2)}
                  </td>
                  <td className="py-3 px-3 text-sm font-bold text-slate-800 text-center">
                    ₹{formatINR(totals.totalExchInAmnt)}
                  </td>
                  <td className="py-3 px-3 text-sm font-bold text-slate-800 text-center">
                    {totals.totalExchOutQty.toFixed(2)}
                  </td>
                  <td className="py-3 px-3 text-sm font-bold text-slate-800 text-center">
                    ₹{formatINR(totals.totalExchOutAmnt)}
                  </td>
                  <td className="py-3 px-3 text-sm font-medium text-slate-700" />
                  <td className="py-3 px-3 text-sm font-medium text-slate-700" />
                </tr>
